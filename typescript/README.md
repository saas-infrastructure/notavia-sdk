...
```typescript
import { verifyWebhookSignature } from "@notavia/sdk/webhooks";
```

Use `verifyWebhookSignature` server-side to verify the `NotifyService-Signature` header on incoming webhook requests before processing the payload. Never call it in the browser — the signing secret must stay on the server.
...
The packages are published on npm. Consumers do not need a tarball:

```powershell
npm install @notavia/sdk
```
