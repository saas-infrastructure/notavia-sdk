...
export class NotifyServiceInbox extends LitElement {
  ...
  @state() loadState: "loading" | "loaded" | "error" = "loading";
  ...
  override render(): TemplateResult {
    switch (this.variant) {
      ...
      case "list":  return renderListVariant(this);
    }
  }
  ...
  async markAllRead(): Promise<void> {
    ...
    const unreadIds = new Set(this.items.filter((i) => i.readAt == null).map((i) => i.id));
    this.markingAllRead = true;
    try {
      const res = await client.markAllRead();
      if (this.client !== client) return;
      const readAt = new Date().toISOString();
      this.items = this.items.map((i) => unreadIds.has(i.id) ? { ...i, readAt } : i);
      this.unreadCount = this.items.filter((i) => i.readAt == null).length;
      this.dispatchEvent(new CustomEvent("notify-marked-all-read", {
        detail: { markedCount: res.markedCount },
        bubbles: true, composed: true,
      }));
    } catch (err) {
      if (this.client === client) this.dispatchError(err);
    } finally {
  ...
  private async bootClient(): Promise<void> {
    if (!this.publishableKey || !this.token) {
