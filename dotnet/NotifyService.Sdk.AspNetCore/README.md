...
ASP.NET Core Minimal API integration for receiving signed webhook events from Notavia.
...
The signing secret is shown once, when you create the endpoint under **Settings → Webhooks** in the Notavia console; it cannot be read back later, so store it as you would any other secret:
...
if you want to suppress errors and still return `200 OK` to Notavia.

Notavia retries delivery when it receives a non-`2xx` response, so returning `200 OK` after
