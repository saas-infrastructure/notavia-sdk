...
import { PrefsAuthError, PrefsServerError } from "./errors.js";
...
export async function httpJson<T>(
  ...
  if (response.status === 401) {
    ...
    catch (err) { throw err instanceof PrefsAuthError ? err : new PrefsAuthError("refresh_failed"); }
    response = await ctx.fetchImpl(url, init(ctx.tokens.current()));
    if (response.status === 401) throw new PrefsAuthError("unauthorized");
  }

  if (!response.ok) {
    let parsed: unknown;
    try { parsed = await response.json(); } catch { /* non-JSON */ }
    throw new PrefsServerError(
      `Prefs request ${method} ${path} failed with ${response.status}`,
      response.status,
      parsed,
    );
  }
